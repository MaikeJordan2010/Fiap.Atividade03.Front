...
namespace Atividade03.Front.Dominio.Validadores
{
    public class ValidarCadastrarContato : AbstractValidator<Contato>
    {
        public ValidarCadastrarContato()
        {
        ...
        {
            var result = await base.ValidateAsync(ValidationContext<Contato>.CreateWithOptions((Contato)model, x => x.IncludeProperties(propertyName)));
            if (result.IsValid)
